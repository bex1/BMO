...
function userLoginMenu() {
    ...
    foreach($menu as $key=>$val) {
        $html .= "<a href='$val' style='font-weight: bold; color: #000000'>$key</a> ";
    }
    $html .= "</nav>";
    return $html;
}
...
function userLoginForm($output=null, $outputClass=null) {
    ...
    if(isset($output)) {
        $output = "<p class='small' style='width:500px;'><output class='$outputClass'>$output</output></p>";
    }
    ...
    if(userisAuthenticated()) {
        $disabled = "disabled";
        $disabledInfo = "<em class='quiet small'>Du är inloggad. <a href='?p=logout'>Logga ut</a>.</em>";
    }
...
<form method="post" action="?p=login">
  <fieldset>
    ...
    <p>
      ...
      <input id="input1" class="textInp" type="text" name="account">
    </p>
    <p>
      <label for="input2">Lösenord:</label><br>
      <input id="input2" class="textInp" type="password" name="password">
    </p>
    <p>
      <input class="loginButton" type="submit" name="doLogin" value="Login" $disabled>
      $disabledInfo
    </p>
  </fieldset>
</form>
...
function userLogin() {
    ...
    if(isset($_POST['doLogin'])) {
        ...
        if($userAccount == $_POST['account'] && $userPassword == userPassword($_POST['password'])) {
            $output = "Välkommern " . $_POST['account'] . "! Du dirigeras nu vidare till admin funktioner.";
            $outputClass = "success";
